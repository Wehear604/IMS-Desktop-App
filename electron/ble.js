...
const { SERVICE_UUID, MANUFACTURER_IDENTIFIER, CHARACTERISTIC_UUID_READ_WRITE, CHARACTERISTIC_UUID_READ_NOTIFY } = require("../src/utils/constants");
...
function send(channel, data) {
    BrowserWindow.getAllWindows().forEach(w => {
        try { w.webContents.send(channel, data); } catch (e) { /* ignore */ }
    });
}
...
Object.keys(MANUFACTURER_IDENTIFIER || {}).forEach(k => {
    try { MANUF_MAP_INT[k] = parseManufHex(MANUFACTURER_IDENTIFIER[k]); } catch (e) { MANUF_MAP_INT[k] = NaN; }
});
...
function pushActivity(deviceId, level, msg, meta) {
    ...
    if (activityStore.__global.length > MAX_LOGS_PER_DEVICE) activityStore.__global.shift();
    ...
    try {
        BrowserWindow.getAllWindows().forEach(w => {
            try { w.webContents.send("ble-activity", entry); } catch (e) { }
        });
    } catch (e) { /* ignore */ }

    if (entry.level === "error") console.error(`[BLE][${entry.ts}][${deviceId}] ${entry.msg}`, entry.meta || "");
    else console.log(`[BLE][${entry.ts}][${deviceId}] ${entry.msg}`, entry.meta || "");
}
function activityInfo(deviceId, msg, meta) { pushActivity(deviceId, "info", msg, meta); }
function activityWarn(deviceId, msg, meta) { pushActivity(deviceId, "warn", msg, meta); }
function activityError(deviceId, msg, meta) { pushActivity(deviceId, "error", msg, meta); }
...
noble.on("discover", (peripheral) => {
    try {
        ...
        activityInfo(peripheral.id, `Discovered device (${name}) rssi=${peripheral.rssi}`, { address: peripheral.address });
        ...
        activityError(null, "Error in discover handler", { err: err && err.message });
...
ipcMain.on("ble-connect-manual", (event, payload) => {
    ...
    p.connect((err) => {
        ...
        const notifyUuidRaw = CHARACTERISTIC_UUID_READ_NOTIFY[deviceType] || CHARACTERISTIC_UUID_READ_NOTIFY[1];
        const writeUuidRaw = CHARACTERISTIC_UUID_READ_WRITE[deviceType] || CHARACTERISTIC_UUID_READ_WRITE[1];
        ...
        p.discoverServices([], (err, services) => {
            if (err) {
                ...
                activityError(p.id, "Full Service discovery error", { err: err && err.message });
            ...
            console.log("Discovered ALL Services:", services.map(s => s.uuid));
            activityInfo(p.id, `Discovered ${services.length} services`, { services: services.map(s => s.uuid) });
            send("ble-status", `Discovered ${services.length} services. Fetching characteristics...`);

            // --- STEP 2: Discover Characteristics ONLY for the target service ---
            const targetService = services.find(s => normalizeUUID(s.uuid) === serviceNormalized);

            if (!targetService) {
                console.error(`Target service ${serviceNormalized} not in the discovered list. Check constants.`);
                rec.discoveryComplete = true;
            ...
            targetService.discoverCharacteristics([], (err, characteristics) => {
                ...
                try { rec.peripheral.characteristics = characteristics; } catch (e) { }
                ...
                if (!notifyChar) notifyChar = characteristics.find(c => normalizeUUID(c.uuid).includes(notifyNormalized));
                if (!writeChar) writeChar = characteristics.find(c => normalizeUUID(c.uuid).includes(writeNormalized));
                ...
                if (notifyChar) {
                    notifyChar.on("data", (data, isNotification) => {
                        ...
                        activityInfo(p.id, `Notify data received: ${hex}`, { isNotification });
                        ...
                        try {
                            ...
                            activityWarn(p.id, "Error parsing notification volume", { err: e && e.message });
                        }
                    });

                    notifyChar.subscribe((err) => {
                        if (!err) {
                            activityInfo(p.id, "Notifications enabled on characteristic", { uuid: notifyChar.uuid });
                            send("ble-status", "Notifications enabled.");
                        } else {
                            activityWarn(p.id, "Failed to enable notifications", { err: err && err.message });
                        }
                    });
                }

                if (writeChar) {
                    activityInfo(p.id, "Write characteristic available", { uuid: writeChar.uuid });
...
ipcMain.on("ble-write", (event, payload) => {
    ...
    if (Buffer.isBuffer(payload.data)) buffer = payload.data;
        ...
        activityWarn(payload.id, "Unsupported write data format", { dataType: typeof payload.data });
    ...
    writeChar.write(buffer, !!payload.withoutResponse, (err) => {
        if (err) {
            ...
            activityError(payload.id, "Write failed", { err: err && err.message, hex: buffer.toString("hex") });
            send("ble-error", `Write failed: ${err.message || err}`);
        } else {
            activityInfo(payload.id, "Write successful", { hex: buffer.toString("hex") });
...
function tryReadChar(char) {
    return new Promise((resolve, reject) => {
        ...
        if (!char.read || typeof char.read !== "function") return reject(new Error("No read function"));
...
function collectReadCandidates(rec, normalizedUuid) {
    ...
    allChars.forEach(c => {
        if (normalizedUuid && normalizeUUID(c.uuid) === normalizedUuid) exact.push(c);
        else others.push(c);
    });
...
function detectChangeInHistory(hexHistory, byteIndexPreference = [1, 0]) {
    ...
    if (bytesList.length <= 1) return { changed: false, reason: "insufficient samples", chosenIndex: null };

    for (const preferredIdx of byteIndexPreference) {
        const vals = bytesList.map(b => (b.length > preferredIdx ? b[preferredIdx] : null));
        if (vals.every(v => v === null)) continue;
        const unique = Array.from(new Set(vals));
        if (unique.length > 1) return { changed: true, chosenIndex: preferredIdx, values: vals };
    }
    ...
    for (let idx = 0; idx < maxLen; idx++) {
        ...
        const unique = Array.from(new Set(vals));
        if (unique.length > 1) return { changed: true, chosenIndex: idx, values: vals };
    }
...
async function readVolumeNowPromise(peripheralId, opts = {}) {
    ...
    if (!rec || !rec.peripheral) throw new Error("Device not found or not connected.");

    const p = rec.peripheral;

    if (opts.debug) activityInfo(peripheralId, "readVolumeNowPromise START", { opts });

    // If discovery hasn't started/complete, attempt to kick off a targeted discover to avoid hang.
    try {
        if (!rec.discoveryComplete) {
            activityInfo(peripheralId, "Discovery not complete - attempting to discover services (targeted) now");
            // attempt targeted discovery quickly (safe if discovery already in progress)
            try {
                await new Promise((resolve, reject) => {
                    p.discoverServices([], (err, services) => {
                        if (err) return reject(err);
                        try { activityInfo(peripheralId, `discoverServices immediate returned ${services.length} services`, { uuids: services.map(s => s.uuid) }); } catch (e) { }
                        resolve(services || []);
                    });
                });
            } catch (e) {
                activityWarn(peripheralId, "Immediate discoverServices attempt failed (nonfatal)", { err: e && e.message });
            }
        }
    } catch (e) {
        // non-fatal
        activityWarn(peripheralId, "Error during forced discoverServices attempt", { err: e && e.message });
    }
    ...
    while (!rec.discoveryComplete && attempts < opts.discoveryMaxAttempts) {
        if (opts.debug) activityInfo(peripheralId, `Waiting for discoveryComplete (${attempts + 1}/${opts.discoveryMaxAttempts})`);
    ...
    if (!rec.discoveryComplete) {
        ...
        activityWarn(peripheralId, "Service discovery timed out. Attempting forced discoverAllServicesAndCharacteristics fallback.");
        if (typeof p.discoverAllServicesAndCharacteristics === "function" && opts.extendDiscoveryWithAllChars) {
            try {
                const [services, characteristics] = await new Promise((resolve, reject) => {
                    p.discoverAllServicesAndCharacteristics((err, svcs, chars) => {
                        ...
                        chars.forEach(c => console.log("CHAR:", c.uuid, " props:", c.properties));
                        console.log("====== END MAP ======");
                    });
                });
                rec.discoveryComplete = true;
                try { rec.peripheral.characteristics = characteristics; } catch (e) { }
                activityInfo(peripheralId, `discoverAllServicesAndCharacteristics returned ${services.length} services and ${characteristics.length} characteristics`, { services: services.map(s => s.uuid), charCount: characteristics.length });
            } catch (e) {
                activityError(peripheralId, "Forced discoverAllServicesAndCharacteristics failed", { err: e && e.message });
                throw new Error("Service discovery timed out. Please check device connection.");
            }
        } else {
            throw new Error("Service discovery timed out. Please check device connection.");
        }
    ...
    try {
        if (rec.peripheral && Array.isArray(rec.peripheral.services)) {
            activityInfo(peripheralId, "Peripheral.services (post-discovery)", { services: rec.peripheral.services.map(s => s.uuid) });
        }
    } catch (e) { }

    const candidates = collectReadCandidates(rec, preferredNormalized);
    if (!candidates || candidates.length === 0) {
        activityWarn(peripheralId, "No readable/notify characteristics available after discovery", { preferredNormalized });
        throw new Error("No readable/notify characteristics available.");
    }

    if (opts.debug) activityInfo(peripheralId, `Read candidates count=${candidates.length}`, { uuids: candidates.map(c => c.uuid), preferredNormalized });
    ...
    for (let i = 0; i < maxRetries; i++) {
        ...
        try {
            activityInfo(peripheralId, `Trying quick read on candidate ${i + 1}/${maxRetries}`, { uuid: char.uuid, properties: char.properties });
            const res = await tryReadChar(char);
            const volume = parseVolumeFromHex(res.hex);
            activityInfo(peripheralId, `Volume read (single): ${volume}`, { uuid: res.uuid, hex: res.hex });
            return Object.assign(res, { volume, changed: false, via: "read" });
        } catch (err) {
            activityWarn(peripheralId, `Quick read failed for candidate ${i + 1}`, { uuid: char && char.uuid, err: err && err.message });
            continue;
        }
    }
    ...
    try {
        ...
        activityInfo(peripheralId, `Starting polling fallback: count=${opts.pollCount}, interval=${opts.pollIntervalMs}ms`, { primaryUuid: primary.uuid });

        for (let i = 0; i < opts.pollCount; i++) {
            try {
                const res = await tryReadChar(primary).catch(err => tryReadCandidatesSequential(pollCandidates));
                history.push(res.hex);
                activityInfo(peripheralId, `Poll ${i + 1}/${opts.pollCount} success`, { hex: res.hex, uuid: res.uuid });
            } catch (err) {
                history.push(null);
                activityWarn(peripheralId, `Poll ${i + 1}/${opts.pollCount} failed`, { err: err && err.message });
            }
        ...
        activityInfo(peripheralId, `Polling result`, { finalVolume, changed: detection.changed, chosenIndex: detection.chosenIndex, history });
        ...
        activityWarn(peripheralId, "Polling fallback failed, attempting notify fallback", { err: err && err.message });
    }

    // Notify fallback
    try {
        let notifyChar = candidates.find(c => (c.properties || []).map(x => String(x).toLowerCase()).includes('notify') || (c.properties || []).map(x => String(x).toLowerCase()).includes('indicate'));
        if (notifyChar) {
            activityInfo(peripheralId, "Using notify fallback", { uuid: notifyChar.uuid });
            ...
            return await new Promise((resolve, reject) => {
                const onData = (data) => {
                    try {
                        ...
                        activityInfo(peripheralId, `Volume via notify fallback: ${volume}`, { uuid: notifyChar.uuid, hex });
                        resolve({ uuid: notifyChar.uuid, hex, volume, changed: true, via: "notify" });
                    } catch (e) {
                        reject(e);
                    }
                };
            ...
            activityWarn(peripheralId, "No notify characteristic available for notify fallback");
        }
    } catch (e) {
        activityError(peripheralId, "Notify fallback error", { err: e && e.message });
    }
...
ipcMain.on("ble-read-volume-now", async (event, payload) => {
    ...
    try {
        ...
        activityInfo(id, `IPC read complete: volume=${result.volume}`, { via: result.via });
        ...
        send("ble-read-volume-now-response", { id, error: err.message || String(err) });
    }
});

// Helper for existing simple read
function readCharacteristic(peripheralId, characteristicUuidNormalized, replyChannel) {
    const rec = discovered[peripheralId];
    ...
    tryReadCandidatesSequential(candidates)
        .then(result => {
            activityInfo(peripheralId, "Read characteristic success", { uuid: result.uuid, hex: result.hex });
            send(replyChannel, { id: peripheralId, uuid: result.uuid, hex: result.hex });
        })
        .catch(err => {
...
ipcMain.on("ble-read-volume", (event, payload) => {
    ...
    const id = payload.id;
    const deviceType = payload.type || (discovered[id] && discovered[id].deviceType) || 1;
    if (payload.uuid) {
        readCharacteristic(id, normalizeUUID(payload.uuid), "ble-read-volume-response");
    } else {
        const volumeUuid = VOLUME_UUID_BY_TYPE[deviceType];
        readCharacteristic(id, normalizeUUID(volumeUuid), "ble-read-volume-response");
    }
});
...
ipcMain.handle("ble-debug-dump", async (event, { id } = {}) => {
    ...
    const safe = {
        ...
        adv: (() => {
            try {
                ...
                return {
                    ...
                    manufacturerDataHex: a.manufacturerData ? Buffer.from(a.manufacturerData).toString("hex") : null,
        ...
        services: (() => {
            try {
                if (!rec.peripheral || !Array.isArray(rec.peripheral.services)) return null;
                return rec.peripheral.services.map(s => ({ uuid: s.uuid, characteristics: (s.characteristics || []).map(c => c.uuid) }));
            } catch (e) { return null; }
        })(),
        chars: (() => {
            try {
                ...
                if (rec.peripheral && Array.isArray(rec.peripheral.characteristics)) {
                    out._peripheral_list = rec.peripheral.characteristics.map(c => ({ uuid: c.uuid, properties: c.properties }));
                }
                return out;
            } catch (e) { return null; }
        })()
